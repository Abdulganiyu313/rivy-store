...
(async () => {
  try {
    ...
    await client.query(`
      ...
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='products_name_unique'
        ) THEN
    ...
    for (const p of rows) {
      await client.query(
        ...
        ON CONFLICT (name) DO UPDATE SET
          ...
          "financingEligible" = EXCLUDED."financingEligible",
          "updatedAt" = NOW();
      `,
        [
          p.name,
